...
global.swissclac.lib.shuntingYard = function () {
...
global.swissclac.lib.shuntingYard.prototype.peekOperand = function () {
	var len = this._operands.length;
	return (len !== 0) ? this._operands[len - 1] : 0.0;
};

// Pops highest value on stack. Returns 0 if empty.
global.swissclac.lib.shuntingYard.prototype.popOperand = function () {
	var len = this._operands.length;
	return (len !== 0) ? this._operands.pop() : 0.0;
};

// Returns number of operands.
global.swissclac.lib.shuntingYard.prototype.numOperands = function () {
	return this._operands.length;
};

// Pops highest operator on stack.
global.swissclac.lib.shuntingYard.prototype.popOperator = function () {
	return this._actionBuffer.pop();
};

// Returns number of operators.
global.swissclac.lib.shuntingYard.prototype.numOperators = function () {
	return this._actionBuffer.length;
};

// Returns true if currently evaluating sub-expression.
global.swissclac.lib.shuntingYard.prototype.inSubExpression = function () {
	return this._numOpenParen > 0;
};

// Clears all stacks.
global.swissclac.lib.shuntingYard.prototype.clear = function () {
...
global.swissclac.lib.shuntingYard.prototype.evaluate = function () {
...
global.swissclac.lib.shuntingYard.prototype.applyOperator = function (operator) {
	var val = operator.evaluate(this);
	this.addOperand(val);
};

// Adds an operand to the stack.
global.swissclac.lib.shuntingYard.prototype.addOperand = function (operand) {
	this._operands.push(operand);
};

// Adds the given operator.
global.swissclac.lib.shuntingYard.prototype.addOperator = function (operator) {
	if (operator.IsOpenParen) {
		this.addOpenParen(operator);
	} else if (operator.IsCloseParen) {
		this.addCloseParen(operator);
	} else if (operator.Arity == global.swissclac.lib.operator.ARITY_NULLARY) {
		this.addNullaryOperator(operator);
	} else if (operator.Arity == global.swissclac.lib.operator.ARITY_UNARY) {
		this.addUnaryOperator(operator);
	} else if (operator.Arity == global.swissclac.lib.operator.ARITY_BINARY) {
		this.addBinaryOperator(operator);
	}
};

// Evaluates the NullaryOperator and pushes result to stack.
global.swissclac.lib.shuntingYard.prototype.addNullaryOperator = function (operator) {
	this.applyOperator(operator);
};

// Evaluates the NullaryOperator and pushes result to stack.
global.swissclac.lib.shuntingYard.prototype.addUnaryOperator = function (operator) {
	this.applyOperator(operator);
};

// First adds operator to _actionBuffer before committing to anything.
global.swissclac.lib.shuntingYard.prototype.addBinaryOperator = function (operator) {
...
global.swissclac.lib.shuntingYard.prototype.addOpenParen = function (operator) {
	this._actionBuffer.push(operator);
	this._numOpenParen++;
};

// Adds the close parenthesis operator. Pops operators until open is reached.
global.swissclac.lib.shuntingYard.prototype.addCloseParen = function (operator) {
